<template>
  <div class="fixed inset-0 z-50 grid place-items-center bg-slate-900/40 backdrop-blur-sm px-4">
    <div class="w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl border border-secondary/20 shadow-2xl p-6 sm:p-8">
      <!-- Header -->
      ...
      <p class="text-slate-500 dark:text-slate-400 text-xs text-center mt-1.5 leading-relaxed">
        Pick a topic or type your own interest
      </p>
      ...
      <button
        ...
        :class="interest.trim() && !isLoading
          ? 'bg-primary text-white shadow-lg shadow-primary/25 hover:shadow-xl hover:shadow-primary/30 active:scale-[0.98]'
          : 'bg-slate-200 dark:bg-slate-700 text-slate-400 dark:text-slate-500 cursor-not-allowed'"
        @click="handleStart"
      >
        <span v-if="isLoading" class="inline-flex items-center gap-2">
          <span class="inline-flex items-center gap-1" aria-hidden="true">
            <i class="w-1.5 h-1.5 rounded-full bg-white/80" style="animation: connecting-bounce 1s ease-in-out infinite"></i>
            <i class="w-1.5 h-1.5 rounded-full bg-white/80" style="animation: connecting-bounce 1s ease-in-out infinite 0.14s"></i>
            <i class="w-1.5 h-1.5 rounded-full bg-white/80" style="animation: connecting-bounce 1s ease-in-out infinite 0.28s"></i>
          </span>
          Generating lesson...
        </span>
      ...
      <!-- Error Message -->
      <p v-if="errorMessage" class="text-red-500 text-xs text-center mt-3">
        {{ errorMessage }}
      </p>
    </div>
  </div>
